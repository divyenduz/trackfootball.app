...
export async function getStats(
  ...
  const res = await request<
    ...
    unknown
  >({ method: 'GET', url: getGetStatsUrl(id).toString(), ...requestConfig })
  return res.data
}
...
export async function starSegment(
  ...
  config: Partial<RequestConfig<StarSegmentMutationRequest>> & {
...
function getGetRoutesByAthleteIdUrl() {
  // @ts-expect-error likely an issue in swagger definition
...
export async function createUpload(
  ...
  config: Partial<RequestConfig<CreateUploadMutationRequest>> & {
    client?: typeof client
  } = {}
) {
  const { client: request = client, ...requestConfig } = config
